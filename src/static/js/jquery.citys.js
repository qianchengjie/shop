...
;(function (factory) {
    ...
    jq.fn.citys = function(parameter,getApi) {
        ...
        var options = jq.extend({}, defaults, parameter);
        return this.each(function() {
            //对象定义
            var _api = {};
            var $this = jq(this);
            var $province = $this.find('div[name="'+options.provinceField+'"]'),
                $city = $this.find('div[name="'+options.cityField+'"]'),
                $area = $this.find('div[name="'+options.areaField+'"]');
            jq.ajax({
                url:options.dataUrl,
                type:'GET',
                ...
                success:function(data){
                    ...
                    var format = {
                        province:function(){
                            ...
                            for(var i in province){
                                ...
                                $province.append('<div value="'+(options.valueType=='code'?i:province[i])+'" data-code="'+i+'" style="background-color: #FFF;" class="option weui-cell vux-tap-active weui-cell_access vux-cell-no-border-intent"><div class="weui-cell__hd">  </div><div class="vux-cell-bd vux-cell-primary"><p><label class="vux-label">'+province[i]+'</label></p><span class="vux-label-desc"></span></div><div class="weui-cell__ft"> </div></div>');
                            }
                            if(options.province){
                        ...
                        city:function(){
                            ...
                            if(!hasCity){
                                ...
                                if(options.nodata=='disabled'){
                                    $city.prop('disabled',jq.isEmptyObject(city));
                                }else if(options.nodata=='hidden'){
                                    $city.css('display',jq.isEmptyObject(city)?'none':'');
                                }
                                for(var i in city){
                                    $city.append('<div value="'+(options.valueType=='code'?i:city[i])+'" data-code="'+i+'" style="background-color: #FFF;" class="option weui-cell vux-tap-active weui-cell_access vux-cell-no-border-intent"><div class="weui-cell__hd">  </div><div class="vux-cell-bd vux-cell-primary"><p><label class="vux-label">'+city[i]+'</label></p><span class="vux-label-desc"></span></div><div class="weui-cell__ft"> </div></div>');
                                }
                                if(options.city){
                        ...
                        area:function(){
                            ...
                            if(options.nodata=='disabled'){
                                $area.prop('disabled',jq.isEmptyObject(area));
                            }else if(options.nodata=='hidden'){
                                $area.css('display',jq.isEmptyObject(area)?'none':'');
                            }
                            for(var i in area){
                                $area.append('<div value="'+(options.valueType=='code'?i:area[i])+'" data-code="'+i+'" style="background-color: #FFF;" class="option weui-cell vux-tap-active weui-cell_access vux-cell-no-border-intent"><div class="weui-cell__hd">  </div><div class="vux-cell-bd vux-cell-primary"><p><label class="vux-label">'+area[i]+'</label></p><span class="vux-label-desc"></span></div><div class="weui-cell__ft"> </div></div>');
                            }
                            if(options.area){
                    ...
                    $province.on('click', '.option', function(){
                        options.province = jq(this).data('code')||0; //选中节点的区划代码
                        options.city = 0;
                        options.area = 0;
                        updateData();
                        format.city();
                    ...
                    $city.on('click', '.option', function(){
                        options.city = jq(this).data('code')||0; //选中节点的区划代码
                        options.area = 0;
                        updateData();
                        format.area();
                        options.onChange(_api.getInfo());
                    ...
                    $area.on('click', '.option', function(){
                        options.area = jq(this).data('code')||0; //选中节点的区划代码
                        options.onChange(_api.getInfo());
                        $area.hide();
                    });
                    //初始化
